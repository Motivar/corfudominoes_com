...
if ($kk>0)
{ /*enable the wrpper with the margin*/
...
add_shortcode( 'reviews_slider', 'function_reviews_slider' );

function function_reviews_slider( $atts )
{
        extract( shortcode_atts( array(
                 'small_header' => '',
                'reviews' => '',
        ...
        $msg = '<div class="sbp_slider">';
        /*initialize div*/
        if ( $small_header != '' ) {
                $msg .= '<div class="sbp_small_header sbp_bot"><h3>' . sanitize_text_field( $small_header ) . '</h3></div>';
                /*initiazize small title*/
        }
        ...
        if ( $reviews == 1 ) {
                ...
                $rs      = get_posts( $reviews );
                if ( !empty( $rs ) ) {
                        $msg .= '<div class="sbp_big_header_container sbp_review sbp_bot">';
                        foreach ( $rs as $r ) {
                                $msg .= '<div class="sbp_review_loop" id="' . $r->ID . '"><h1>' . sbp_limit_text( $r->post_title, $word_limit ) . '</h1></div>';
                        }
                ...
                $otas = array();
                $otas[] = array('7.9','10','Booking.com');
                $otas[] = array('4.5', '5','TripAdvisor');

                $count = 0;
                $msg .= '<div class="sbp_ota_container sbp_bot">';
                foreach ($otas as $ota) {
                        $count = $count + 1;
                        $msg .= '<div class="sbp_ota_loop" id="' . $count . '"><h3><span class="name">' . $ota[2] . $marker . ' </span><span class="grade">' . $ota[0] . '</span><span class="max_grade">/' . $ota[1] . '</span></h3></div>';
                }
                $msg .= '</div>';
        }
        ...
        /*show button*/
            if ($show_min==1)
            {
                $min = get_option( 'sbp_global_min_price' ) ?: '';
                $button=$min != '' ? __('Book Now From', 'simple-bookings-plugin') .' '.$min .' '.$currency : __('Book Now', 'simple-bookings-plugin');
            }
            $msg .= '<div class="sbp_bot"><a class="mediumbtn sbp_book_now" href="' . sbp_booking_post_url . '">' . $button . '</a></div>';
        /*show more button for scroll to thing*/
        if ( !empty( $scroll_button ) && !empty($scroll_div) ) {
                $arrow = do_shortcode('[icon name= "icon-arrow-down7" size= "small" style= "plain" bg_color= "" hover_bg_color= "" color= "#ffffff" hover_color= "#ffffff" border_width= "1" border_color= "#ffffff" hover_border_color= "" alignment= "center" href= "" animation_type= "fadeIn"]');
                $msg .= '<div class="sbp_scroll_down" data-to="'.$scroll_div.'"><span>'.$scroll_button.'</span><span>'.$arrow.'</span></div>';
        }
